...
var (
	...
	plotw, ploth, gwidth, gheight, gutter, beginx, beginy float64
	plotc                                                 int
)

const (
	globalfmt = "font-family:%s;font-size:%vpt;stroke-width:%vpx"
	linestyle = "fill:none;stroke:"
	linefmt   = "fill:none;stroke:%s"
	barfmt    = linefmt + ";stroke-width:%vpx"
...
func init() {
	...
	ymin := flag.Float64("ymin", smallint, "y minimum")
	ymax := flag.Float64("ymax", smallint, "y maximum")

	// meta options
	flag.Float64Var(&beginx, "bx", 100, "initial x")
	flag.Float64Var(&beginy, "by", 50, "initial y")
...
func plot(x, y, w, h float64, settings plotset, d []rawdata) {
	...
	for i, v := range d {
		xp := fmap(v.x, minx, maxx, float64(x), float64(x+w))
		yp := fmap(v.y, miny, maxy, float64(y), float64(y-h))
		...
		if settings.opt["showx"] {
			if i%int(settings.size["xinterval"]) == 0 {
				canvas.Text(xp, (y+h)+(spacer*2), fmt.Sprintf("%0.f", v.x), "text-anchor:middle")
	...
	if settings.opt["showy"] {
		...
		for yax := bot; yax <= top; yax += interval {
			...
			canvas.Text(x-spacer, yaxp+h, fmt.Sprintf("%.1f", yax), textfmt)
			canvas.Line(x-spacer, yaxp+h, x, yaxp+h)
		}
		canvas.Gend()
	}
