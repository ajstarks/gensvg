...
const (
	...
	rotextfmt = "fill:%s;font-family:%s;font-size:%vpt"
	flowerfmt = "stroke:rgb(%d,%d,%d); stroke-opacity:%.2f; stroke-width:%.2f"
	tilestyle = "stroke-width:1; stroke:rgb(128,128,128); stroke-opacity:0.5; fill:white"
	penstyle  = "stroke:rgb%s; fill:none; stroke-opacity:%.2f; stroke-width:%.2f"
	width     = 256.0
	height    = 256.0
)
...
// qfint returns the float64 value of a query string, within limits
...
// random returns a floating point number in the specified range
...
func rotext(w http.ResponseWriter, req *http.Request) {
	...
	ti := qfloat(query, "ti", 10, 5, 360)       // angle float64erval
...
func rshape(w http.ResponseWriter, req *http.Request) {
	...
	for i := 0; i < n; i++ {
		...
		if samesize {
			...
			s2 = random(0, height/5)
		}
		shapefunc(random(0, width), random(0, height), s1, s2,
			fmt.Sprintf("fill-opacity:%.2f;fill:rgb(%d,%d,%d)",
				rand.Float64(), rand.Intn(255), rand.Intn(255), rand.Intn(255)))
	}
	canvas.End()
}
...
func flower(w http.ResponseWriter, req *http.Request) {
	...
	np := qfloat(query, "petals", 15, 10, 60)      // number of "petals" per flower
	opacity := qfloat(query, "op", 50, 20, 100)    // opacity
	psize := qfloat(query, "size", 30, 5, 50)      // length of the petals
	thickness := qfloat(query, "thick", 10, 3, 20) // petal thickness
	...
	for i := 0; i < n; i++ {
		...
		for theta := 0.0; theta < limit; theta += limit / random(10, np) {
			xr := r * math.Cos(theta)
			yr := r * math.Sin(theta)
			canvas.Line(x, y, x+xr, y+yr)
		}
		canvas.Gend()
	}
	canvas.End()
...
func rcube(canvas *gensvg.SVG, x, y, l float64) {

	// top face
	tx := []float64{x, x + (l * 3), x, x - (l * 3), x}
	ty := []float64{y, y + (l * 2), y + (l * 4), y + (l * 2), y}

	// left face
	lx := []float64{x - (l * 3), x, x, x - (l * 3), x - (l * 3)}
	ly := []float64{y + (l * 2), y + (l * 4), y + (l * 8), y + (l * 6), y + (l * 2)}

	// right face
	rx := []float64{x + (l * 3), x + (l * 3), x, x, x + (l * 3)}
	ry := []float64{y + (l * 2), y + (l * 6), y + (l * 8), y + (l * 4), y + (l * 2)}
...
func cube(w http.ResponseWriter, req *http.Request) {
	...
	hspace := qfloat(query, "hs", width/5, 0, width)    // horizontal space
	vspace := qfloat(query, "vs", height/4, 0, height)  // vertical space
	size := qfloat(query, "size", width/30, 2, width/4) // cube size
	xp := qfloat(query, "x", width/10, 0, width/2)      // initial x position
	yp := qfloat(query, "y", height/10, 0, height/2)    // initial y position
...
func lew(canvas *gensvg.SVG, x float64, y float64, gsize float64, n int, w float64) {
...
func lewitt(w http.ResponseWriter, req *http.Request) {
	...
	nc := width / gsize
	nr := height / gsize
	for cols := 0.0; cols < nc; cols++ {
		for rows := 0.0; rows < nr; rows++ {
...
func funnel(w http.ResponseWriter, req *http.Request) {
	...
	grid := qfloat(query, "step", 25, 10, height/3)
	h := width / 2
...
const (
	...
	linefmt  = "stroke:%s;stroke-width:%.2f"
	digitfmt = "font-family:Helvetica,Calibri,sans-serif;text-anchor:middle;font-size:%vpx"
)
...
func clock(w http.ResponseWriter, req *http.Request) {
	...
	for h := 12; h > 0; h-- {
		...
		x := rx + r*math.Cos(t)
		y := ry + r*math.Sin(t)
		canvas.Text(x, height-y, digits[h%12], "baseline-shift:-30%")
	}
	canvas.Gend()
	...
	canvas.Line(cx, cy, float64(hx), height-hy, fmt.Sprintf(linefmt, hrcolor, basesize))
	canvas.Line(cx, cy, float64(mx), height-my, fmt.Sprintf(linefmt, mincolor, basesize/2))
	canvas.Line(cx, cy, float64(sx), height-sy, fmt.Sprintf(linefmt, secolor, basesize/4))
...
func tux(w http.ResponseWriter, req *http.Request) {
	...
	beakx := []float64{58, width - 58, width / 2}
	beaky := []float64{height - 62, height - 62, height - 20}
