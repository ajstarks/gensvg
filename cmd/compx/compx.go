...
type Component struct {
	...
	Gutter float64  `xml:"gutter,attr"`
	Gw     float64  `xml:"gw,attr"`
	Gh     float64  `xml:"gh,attr"`
...
type group struct {
	Brow    float64 `xml:"brow,attr"`
	Bcol    float64 `xml:"bcol,attr"`
	Erow    float64 `xml:"erow,attr"`
	Ecol    float64 `xml:"ecol,attr"`
...
type note struct {
	Row     float64 `xml:"row,attr"`
	Col     float64 `xml:"col,attr"`
	Width   float64 `xml:"width,attr"`
	Height  float64 `xml:"height,attr"`
...
var (
	width, height, fontscale                             float64
	linesize, labelfs, notchsize, groupmargin            float64
...
const (
	...
	globalstyle = "font-family:Calibri,sans-serif;font-size:%vpx;fill:black;text-anchor:middle;stroke-linecap:round;stroke-width:%vpx;stroke-opacity:%s"
	ltstyle     = "text-anchor:%s;fill:black"
	legendstyle = "text-anchor:start;fill:black;font-size:%vpx"
	gridstyle   = "fill:none; stroke:gray; stroke-opacity:0.3"
	notefmt     = "font-size:%vpx"
...
func lookup(id string, c []comp, t, l, g float64) gcomp {
	var x gcomp
	for _, v := range c {
		if id == v.Id {
			return gc(v, t, l, g)
...
func dotitle(left, offset float64, t string) {
...
func grid(top, left, gutter float64) {
	gs := strings.SplitN(guide, `x`, 4)
	if len(gs) != 4 {
		return
	}
	w, _ := strconv.ParseFloat(gs[0], 64)
	h, _ := strconv.ParseFloat(gs[1], 64)
	nr, _ := strconv.ParseFloat(gs[2], 64)
	nc, _ := strconv.ParseFloat(gs[3], 64)
	...
	for r := 0.0; r < nr; r++ {
		...
		for c := 0.0; c < nc; c++ {
			canvas.Rect(x, y, w, h)
			canvas.Text(x+w/2, y+h/2, fmt.Sprintf("%v,%v", r, c),
...
func dogroup(g group, top, left, gutter float64) {
	margin := groupmargin
...
func dolegend(leg legend, top, left, gutter, fs, ls float64) {
	if leg.Size > 0 {
		fs = leg.Size
		ls = fs + 4
	}
...
func donote(n note, top, left, gutter float64) {
...
func gc(c comp, top, left, gutter float64) gcomp {
	var g gcomp
...
func display(c comp, top, left, gutter float64) {
...
func component(g gcomp, c comp) {
	...
	if len(c.Image) > 0 {
		canvas.Image(x, y, int(w), int(h), c.Image)
		if len(c.Name) > 0 {
			canvas.Text(x+w2, y+h/3, c.Name,
				fmt.Sprintf("font-size:%vpx;fill:%s;baseline-shift:50%%", fs, boxcolor))
		}
		return
	}
	...
	case "mobile", "screen":
		...
		canvas.Gstyle(fmt.Sprintf("font-size:%vpx", fs))
	...
	case "server":
		...
		canvas.Text(x+w/2, y+h-10, c.Name, fmt.Sprintf("font-size:%vpx;fill:white", fs))
	...
	case "message":
		...
		canvas.Gstyle(fmt.Sprintf("font-size:%vpx;fill:white", fs))
		if len(c.Os) > 0 {
			canvas.Text(x+w2, y+pmy+fs, c.Os, "font-size:75%")
		}
		canvas.Text(x+w2, y+h-10, c.Name, fmt.Sprintf("font-size:%vpx;fill:%s", fs, bgcolor))
		canvas.Gend()
	...
	case "cloud":
		...
		canvas.Text(xc-(r/4), yc+r/2, c.Name, fmt.Sprintf("font-size:%vpx;fill:white", fs))

	case "db":
		cylinder(x, y+(h/4), w, h-(h/2), h/4, lcolor, boxcolor)
		canvas.Text(x+w2, y+h3, c.Name, fmt.Sprintf("font-size:%vpx;fill:white", fs))
		if len(c.Sw) > 0 {
	...
	case "face":
		...
		canvas.Text(x+w/2, y+(h/2)+fr+15, c.Name, fmt.Sprintf("font-size:%vpx;fill:black", fs))

	case "role":
		role(x, y, w, h, boxcolor)
		canvas.Text(x+w/2, y+h-5, c.Name, fmt.Sprintf("font-size:%vpx;fill:%s", fs, bgcolor))
	...
	case "plain":
		...
		canvas.Text(x+w2, y+(h/2), c.Name, fmt.Sprintf("font-size:%vpx;fill:white;baseline-shift:-25%%", fs))
...
func uselibrary(x, y, w, h float64, name string) {
	canvas.Use(x, y, name, fmt.Sprintf(`width="%v"`, w), fmt.Sprintf(`height="%v"`, h))
}

// Object functions
func cylinder(x, y, w, h, eh float64, fill, tfill string) {
	f := "fill:" + fill
	tf := "fill:" + tfill
...
// folder object
func folder(x, y, w, h, l float64, bcolor, color string) {
	nl := w / 10
	xl := x + nl
	xw := x + w
	yl := y + nl
	...
	lh := nl / 2
	var (
		xn = []float64{xl, xl + l2, xl + l3, xw, xw, xl}
		yn = []float64{y, y, y + lh, y + lh, yh, yh}
		xf = []float64{xw, xw - l, x, x + l}
		yf = []float64{yh, yl + lh, yl + lh, yh}
	)
...
// message object
func message(x, y, w, h, l float64, bcolor, scolor string) {
	et := h / 3
	w2 := w / 2
	px := w / 8
	py := h / 8
	e1x := []float64{x, x, x + w, x + w, x + w2, x}
	e1y := []float64{y + et, y + h, y + h, y + et, y + (et * 2), y + et}
	e2x := []float64{x, x + w2, x + w, x + w2, x}
	e2y := []float64{y + et, y, y + et, y + (et * 2), y + et}
...
// eaec person object
func eaec(x, y, w, h, l float64, scolor, bcolor string) {
	wu := w / 8
	hu := h / 12
	wh := w / 2
	hh := h / 2
	sx := []float64{x + wu*2, x + wu*6, x + wh}
	sy := []float64{y + hu*6, y + hu*6, y + hu*11}
	tx := []float64{x + wh, x + wh + wu, x + wh, x + wh - wu, x + wh}
	ty := []float64{y + hu*6, y + hu*7, y + hu*10, y + hu*7, y + hu*6}
...
// screen object
func screen(x, y, w, h, l float64, bcolor, color string) {
	canvas.Roundrect(x, y, w, h, l, l, "fill:"+bcolor)
	canvas.Rect(x+l, y+l, w-(l*2), h-(l*2), "fill:"+color)
}

// kb (keyboard) object
func kb(x, y, w, h, l float64, color string) {
	var xp = []float64{x + l, x, x + w, x + w - l}
	var yp = []float64{y, y + h, y + h, y}
	canvas.Polygon(xp, yp, "fill:"+color)
}

// desktop object
func desktop(x, y, w, h, l float64, bcolor, color string) {
	screen(x, y, w, h-l*3, l, bcolor, color)
	kb(x, y+h-(l*2), w+l, l*2, l*2, bcolor)
}

// face object
func face(x, y, r, l float64, color, fcolor string) {
	fu := r / 10 // "face unit"
	ep := 3 * fu
	my := y + ep
	canvas.Circle(x, y, r, fmt.Sprintf("fill:%s;stroke-width:%vpx;stroke:%s", fcolor, l, color))
	canvas.Circle(x+ep, y-ep, r/10, "fill:"+color)
	canvas.Circle(x-ep, y-ep, r/10, "fill:"+color)
	canvas.Qbez(x+ep, my, x, y+ep*2, x-ep, my, fmt.Sprintf("fill:%s;stroke-width:%vpx;stroke:%s", color, l, color))
}

// server object
func server(x, y, w, h, l float64, bcolor, color string) {
	var xp = []float64{x + (l * 2), (x + w) - (l * 2), (x + w) - l, x + l}
	var yp = []float64{y, y, y + l, y + l}
	canvas.Polygon(xp, yp, "fill:"+color)
...
// role object
func role(x, y, w, h float64, color string) {
	hs := h / 20
	var xp = []float64{x, x, x + w/3, x + w/2, x + (w / 2) + (w / 6), x + w, x + w}
	var yp = []float64{y + h, y + (16 * hs), y + (12 * hs), y + (14 * hs), y + (12 * hs), y + (16 * hs), y + h}

	// var xp = []float64{x, x, x + w/2, x + w, x + w}
	// var yp = []float64{y + h, y + (h5 * 4), y + (h5 * 2), y + (h5 * 4), y + h}
...
func sloper(x1, y1, x2, y2 float64) (m, r float64) {
	dy := float64(y1 - y2)
	dx := float64(x1 - x2)
	m = dy / dx
...
func rowy(n, h, g, t float64) float64 { return t + (n * g) + (n * h) }

// colx computes the x position of a column
func colx(n, w, g, l float64) float64 { return l + (n * g) + (n * w) }

// compass returns the coordinates of a compass point
func compass(g gcomp, point string) (cx, cy float64, dir string) {
	switch point {
	case "nw":
		cx, cy, dir = g.x, g.y, "r"
	case "nnw":
...
func linelabel(x1, y1, x2, y2 float64, label string, mark string, d1 string, d2 string, dir string, color string) {
	aw := linesize * 4
	ah := linesize * 3
...
func doline(x1, y1, x2, y2 float64, style, direction, label string) {
...
func wordstack(x, y, fs float64, s []string, style ...string) {
	ls := fs + 2
	for i := len(s); i > 0; i-- {
...
func arrow(x, y, w, h float64, dir string, color string) (xl, yl float64) {
	var xp = []float64{x, x, x, x}
	var yp = []float64{y, y, y, y}
...
func init() {
	...
	flag.Float64Var(&linesize, "l", 4, "line weight")
	flag.Float64Var(&labelfs, "lf", 16, "label font size (px)")
	flag.Float64Var(&fontscale, "f", 10, "font scaling factor")
	flag.Float64Var(&notchsize, "n", 0, "arrow notch size")
	flag.Float64Var(&groupmargin, "gm", 10, "group margin")
