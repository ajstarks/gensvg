...
// of volleys
func ltposter(x, y float64, w, h float64, f []string) {
	iw := int(w)
	ih := int(h)
	canvas.Image(x, y, iw*2, ih*2, f[0]) // first file, assumed to be the banner
	y = y + (float64(h) * 2)
	for i := 1; i < len(f); i += 2 {
		canvas.Image(x, y, iw, ih, f[i])
		canvas.Image(x+w, y, iw, ih, f[i+1])
		if i%2 == 1 {
			y += h
		}
	}
...
func ltcol(x, y float64, w, h float64, f []string) {
	for i := 0; i < len(f); i++ {
		canvas.Image(x, y, int(w), int(h), f[i])
		y += h
	}
}
...
func ltop(x, y float64, w, h float64, f []string) {
	for i := 1; i < len(f); i++ { // skip the first file, assumed to be the banner
		canvas.Image(x, y, int(w), int(h), f[i], "opacity:0.2")
	}
}

// ltrow creates a row-wise view of volley images.
func ltrow(x, y float64, w, h float64, f []string) {
	for i := 0; i < len(f); i++ {
		canvas.Image(x, y, int(w), int(h), f[i])
		x += w
	}
...
func main() {
	...
	for i, dir := range flag.Args() {
		...
		if len(filelist) != ni || filelist == nil {
			fmt.Fprintf(os.Stderr, "in the %s directory, need %v images, read %d\n", dir, ni, len(filelist))
			continue
		}
		switch {
