...
var (
	width, height, fontsize, barheight, gutter, cornerRadius         float64
	labelimit                                                        int
...
// dobc does file i/o
func dobc(location string, s *gensvg.SVG) {
	var f *os.File
	var err error
	if len(location) > 0 {
...
func readbc(r io.Reader, s *gensvg.SVG) {
	var bc Barchart
...
func drawbc(bg Barchart, canvas *gensvg.SVG) {
...
func makebar(x, y, w, h, radius float64, color string, op float64, canvas *gensvg.SVG) {
	if len(color) > 0 {
...
func makestick(x, y, w float64, color string, canvas *gensvg.SVG) {
	y += fontsize * 60 / 100
...
func main() {
	...
	canvas.Gstyle(fmt.Sprintf(gstyle, int(fontsize)))
	if len(flag.Args()) == 0 {
		dobc("", canvas)
	} else {
