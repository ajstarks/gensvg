...
type geometry struct {
	top, left, width, height, vwidth, vp, barHeight float64
...
func process(canvas *gensvg.SVG, filename string, g geometry) float64 {
	if filename == "" {
...
func (g *geometry) visualize(canvas *gensvg.SVG, filename string, f io.Reader) float64 {
	var (
		err               error
	...
	canvas.Gstyle(fmt.Sprintf("font-size:%vpx;font-family:sans-serif", bh))
	if g.title == "" {
		bmtitle = filename
	} else {
		bmtitle = g.title
	...
	for x, y, nr := g.left+g.vp, g.top+vspacing, 0; err == nil; nr++ {
		...
		bw := vmap(av, dmin, dmax, 0, float64(vizwidth))
		switch g.style {
		case "bar":
...
func (g *geometry) inline(canvas *gensvg.SVG, x, y, w, h float64, bmtype, name, value string, v float64) {
	var color string
	switch bmtype {
	case "delta":
		if v > 0 {
...
func (g *geometry) bars(canvas *gensvg.SVG, x, y, w, h, vs float64, bmtype, name, value string, v float64) {
...
func main() {
	var (
		...
		top          = flag.Float64("top", 50, "top")
		left         = flag.Float64("left", 100, "left margin")
		vp           = flag.Float64("vp", 512, "visualization point")
		vw           = flag.Float64("vw", 300, "visual area width")
		bh           = flag.Float64("bh", 20, "bar height")
