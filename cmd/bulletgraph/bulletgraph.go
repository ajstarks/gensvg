...
var (
	width, height, fontsize, barheight, gutter, circleradius float64
...
// dobg does file i/o
func dobg(location string, s *gensvg.SVG) {
	var f *os.File
	var err error
	if len(location) > 0 {
...
func readbg(r io.Reader, s *gensvg.SVG) {
	var bg Bulletgraph
...
func drawbg(bg Bulletgraph, canvas *gensvg.SVG) {
	...
	for _, v := range bg.Bdata {
		...
		for sc := scalemin; sc <= scalemax; sc += scaleincr {
			...
			canvas.Text(x+scx, y+scalesep+barheight+fontsize/2, fmt.Sprintf(scfmt, sc))
		}
		canvas.Gend()
		...
		for _, q := range qmeasures {
			...
			canvas.Rect(x, y, qbarlength, barheight)
		}
		canvas.Gend()

		// draw the measure and the comparative measure
		barlength := vmap(v.Measure, scalemin, scalemax, 0, maxwidth)
		canvas.Rect(x, y+qmheight, barlength, qmheight, "fill:"+datacolor)
		cmx := vmap(v.Cmeasure, scalemin, scalemax, 0, float64(maxwidth))
		if circlemark {
...
func init() {
	...
	flag.Float64Var(&barheight, "bh", 32, "bar height")
	flag.Float64Var(&circleradius, "cr", 8, "circle radius")
	flag.Float64Var(&gutter, "g", 36, "gutter")
