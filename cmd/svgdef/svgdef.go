...
var (
	...
	oc1      = gensvg.Offcolor{Offset: 0, Color: "white", Opacity: 1.0}
	oc2      = gensvg.Offcolor{Offset: 25, Color: "lightblue", Opacity: 1.0}
	oc3      = gensvg.Offcolor{Offset: 75, Color: "blue", Opacity: 1.0}
	oc4      = gensvg.Offcolor{Offset: 100, Color: objcolor, Opacity: 1.0}
	ga       = []gensvg.Offcolor{oc1, oc2, oc3, oc4}
)

// defcoodstr defines coordinate strings: (x,y)
func defcoordstr(x, y float64, s string) {
...
func defellipse(id string, w, h float64, legend string) {
	canvas.Gid(id)
	canvas.Translate(w, h)
	defcoord(0, 0, -textsize)
...
func defrect(id string, w, h float64, legend string) {
	canvas.Gid(id)
	defcoord(0, 0, -textsize)
...
func defcrect(id string, w, h float64, legend string) {
	canvas.Gid(id)
...
func defimage(id string, w, h float64, s string, legend string) {
	canvas.Gid(id)
	defcoord(0, 0, -textsize)
	canvas.Rect(0, 0, w, h, objstyle)
	canvas.Text(-textsize, (h / 2), "h", legendstyle)
...
func defline(id string, w, h float64, legend string) {
	canvas.Gid(id)
...
func defarc(id string, w, h float64, legend string) {
	canvas.Gid(id)
...
func defbez(id string, x, y, h float64, legend string) {
	sx, sy := 0.0, 0.0
	cx, cy := x, -y
	px, py := x, y
	ex, ey := x*2, 0.0
...
func defqbez(id string, px, py, h float64, legend string) {
	sx, sy := 0.0, 0.0
	ex, ey := px*2, 0.0
...
func defroundrect(id string, w, h float64, rx float64, ry float64, legend string) {
	canvas.Gid(id)
	defcoord(0, 0, -textsize)
...
func defpolygon(id string, w, h float64, legend string) {
...
func defpolyline(id string, w, h float64, legend string) {
...
func deflg(id string, w, h float64, legend string) {
	canvas.Gid(id)
...
func defrg(id string, w, h float64, legend string) {
	canvas.Gid(id)
...
func defmeta(id string, w float64, name, desc []string, legend string) {
	...
	deflegend(w, 0, float64(30*len(name)), legend)
	canvas.Gend()
}
...
func defobjects(w, h float64) {
	...
	defimage("image", 128, 128, "gophercolor128x128.png", "Image(x, y, w, h, float64 path string, style ...string)")
...
func main() {
	...
	canvas.Link("http://github.com/ajstarks/gensvg", "SVGo Library")
	canvas.Text(width/2, 150, "SVG Go Library", "font-size:125px")
	canvas.Text(width/2, 200, "github.com/ajstarks/gensvg", "font-size:50px;fill:gray")
	canvas.LinkEnd()
