...
var (
	byrow                                   bool
	startx, starty, gutter, gwidth, gheight float64
	count                                   int
...
func placepic(x, y float64, filename string) (float64, float64) {
	...
	canvas.Group(`clip-path="url(#pic)"`, fmt.Sprintf(`transform="translate(%v,%v)"`, x, y))
...
func compose(x, y float64, n int, rflag bool, files []string) {
	px := x
	py := y
	var pw, ph float64
	for i, f := range files {
