...
var (
	width, height, fontsize, fontscale, round, gutter, oflen                    float64
	bgcolor, olcolor, colorspec, title                                          string
	pred, pgreen, pblue                                                         int
...
const (
	globalfmt   = "stroke-width:1;font-family:Calibri,sans-serif;text-anchor:middle;font-size:%vpt"
...
func pmap(x, y, fs float64, m Pdata) {
	...
	for i, p := range m.Item {
		...
		dy := yh + fs + (fs / 2)
		if pct <= ofpct || float64(len(k)) > oflen { // overflow label
			if up {
				dy -= loffset
				yh -= loffset
...
func init() {
	...
	flag.Float64Var(&fontsize, "f", 12, "font size (pt)")
	flag.Float64Var(&fontscale, "s", 5, "font scaling factor")
	flag.Float64Var(&round, "r", 0, "rounded corner size")
	flag.Float64Var(&gutter, "g", 100, "gutter")
	flag.Float64Var(&oflen, "ol", 20, "overflow length")
