...
var (
	width, height, iscale, fontsize, margin float64
...
func drawrr(rm Roadmap) {
	...
	for i := 0.0; i < duration; i++ {
		...
		canvas.Text(cx, (cy - sr), fmt.Sprintf("%s %.0f", rm.Unit, i+1), "font-size:150%;fill:gray")
		sr += r
	}
...
func dpolar(cx, cy float64, r, d float64) (float64, float64) {
...
func drawseclines(cx, cy float64, size, a, h float64, s string) {
...
func drawitem(x, y, isize, ieffort float64, s string) {
	var op float64
	if ieffort > 0 {
...
func textlines(x, y, fs, leading float64, align, fill string, s []string) {
	canvas.Gstyle(fmt.Sprintf("font-size:%vpx;text-anchor:%s;fill:%s", fs, align, fill))
	for _, v := range s {
		canvas.Text(x, y, v)
		y += leading
	}
...
func init() {
	...
	flag.Float64Var(&width, "w", 800, "width")
	flag.Float64Var(&height, "h", 800, "height")
	flag.Float64Var(&fontsize, "f", 12, "fontsize (px)")
	flag.Float64Var(&iscale, "s", width*.009, "impact scale")
	flag.Float64Var(&margin, "m", 150, "outside margin")
