...
type Parameters struct {
	...
	x, y, w, h, width, height, spacing, fontsize, dot               float64
...
func (p *Parameters) barchart(location string, canvas *gensvg.SVG) {
	var (
		f   *os.File
		err error
		sp  StockProduct
	...
	bottom := p.y + p.h
	interval := p.w / float64(len(sp.Sdata)-1)
	...
	if p.showgrid {
		...
		for i := p.maxvalue; i >= p.minvalue; i -= p.ginterval {
			yp := vmap(i, p.minvalue, p.maxvalue, float64(p.y), float64(bottom))
			by := p.y + (bottom - yp)
	...
	canvas.Gstyle(fmt.Sprintf("stroke-opacity:%.2f;stroke:%s;stroke-width:%.2f;text-anchor:middle", p.opacity, p.barcolor, bw))
	for _, d := range sp.Sdata {
		yp := vmap(d.Price, p.minvalue, p.maxvalue, float64(p.y), float64(bottom))
		by := p.y + (bottom - yp)
		if p.showline {
		...
		if p.showimage {
			if len(d.Image) > 0 {
				canvas.Image(p.x-bw/2, by-offset-p.fontsize, int(bw), int(offset), d.Image)
			}
		}
...
func init() {
	...
	flag.Float64Var(&param.width, "w", 1600, "overall width")
	flag.Float64Var(&param.height, "h", 900, "overall height")
	flag.Float64Var(&param.x, "left", 150, "left")
	flag.Float64Var(&param.y, "top", 120, "top")
	flag.Float64Var(&param.w, "gw", 1400, "graph width")
	flag.Float64Var(&param.h, "gh", 700, "graph height")
	flag.Float64Var(&param.dot, "dot", 0, "dotsize")
	flag.Float64Var(&param.fontsize, "fs", 14, "font size (px)")
	flag.Float64Var(&param.spacing, "spacing", 15, "bar spacing")
...
func main() {
	...
	canvas.Gstyle(fmt.Sprintf("font-family:Calibri,sans-serif;font-size:%vpx", param.fontsize))
	if len(flag.Args()) == 0 {
